...
void CDistributionChart::paintEvent(QPaintEvent*)
{
	...
	int nLabelWidth = painter.fontMetrics().horizontalAdvance("1200") + 4;
	...
	for (int nLine = 0; 4 > nLine; ++nLine)
	{
		...
		painter.drawText(QRect(0, nY - 8, nLabelWidth, 16), Qt::AlignRight, QString::number(nLine * 400));
	}
	double fStep = area.width() / static_cast<double>(m_values.size());
	for (int nIndex = 0; m_values.size() > nIndex; ++nIndex)
	{
		painter.fillRect(QRectF(area.left() + nIndex * fStep + 2, area.bottom() - m_values[nIndex] * area.height() / 1200.0, fStep - 4, m_values[nIndex] * area.height() / 1200.0), 6 > nIndex ? m_fallingColor : m_risingColor);
	}
